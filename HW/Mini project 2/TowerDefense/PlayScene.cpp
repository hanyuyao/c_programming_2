...
void PlayScene::Update(float deltaTime) {
	...
	// TODO 2 (8/8): Enable the creation of the 4th enemy.
        case 4:
			...
			break;
        case 5:
			EnemyGroup->AddNewObject(enemy = new BEnemy(SpawnCoordinate.x, SpawnCoordinate.y));
			break;
		default:
			continue;
		}
...
void PlayScene::UIBtnClicked(int id) {
	...
	else if (id == 3 && money >= ElectricTurret::Price)
		preview = new ElectricTurret(0, 0);
	if (!preview)
		return;
	preview->Position = Engine::GameEngine::GetInstance().GetMousePosition();
...
std::vector<std::vector<int>> PlayScene::CalculateBFSDistance() {
	...
	std::queue<int> valQue;     //store corresponding value of Point in que
	valQue.push(0);
