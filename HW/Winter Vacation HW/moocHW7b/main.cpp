...
int compare(const void *a, const void *b)
{
    ...
    int k = my_strcmp(ia->name, ib->name);
    if(k != 0)
        return k;
    else if(ia->review != ib->review){
        if(ib->review - ia->review > 0)
            return 1;
        else if(ib->review - ia->review < 0)
            return -1;
        else
            return 0;
    }
    else{
        if(ia->price - ib->price > 0)
            return 1;
        else if(ia->price - ib->price < 0)
            return -1;
        else
            return 0;
    }
